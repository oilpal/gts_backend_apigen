/*
 * Source code generated by UnvUS.
 * Copyright(c) 2017 unvus.com All rights reserved.
 * Template skrull-pack-mybatis:src/main/java/domain/EntityBase.e.vm.java
 * Template is part of project: https://git.unvus.com/unvus/opensource/pack-unvus-mybatis
 */
package com.neofect.gts.core.services.am.domain;

import java.io.Serializable;
import java.time.LocalDateTime;
import java.util.HashMap;
import java.util.Map;

import org.apache.ibatis.type.Alias;

import lombok.Data;

/**
 * @see com.neofect.gts.core.services.am.domain.AmBdgtDrawup
 */
@Alias("am_amBdgtDrawup_")
@Data
public abstract class AmBdgtDrawup_ implements Serializable {

    private static final long serialVersionUID = 1L;

    // Raw attributes
    protected String companyCode;
    protected String deptCode;
    protected String projCode;
    protected String acntCode;
    protected String drawupYymm;
    protected String approveYn;
    protected Long orgAmt;
    protected Long confirmAmt;
    protected Long addAmt;
    protected Long adjustAmt;
    protected Long resultAmt;
    protected Long execAmt;
    protected String bdgtQuarter;
    protected Long quarterAmt;
    protected String approveDutyId;
    protected String approveDate;
    protected String remark;
    protected String inputDutyId;
    protected String inputDate;
    protected String chgDutyId;
    protected String chgDate;
    protected String rowInputEmpNo;
    protected LocalDateTime rowInputDate;
    protected String rowInputIp;
    protected String rowUpdateEmpNo;
    protected String rowUpdateDate;
    protected String rowUpdateIp;

    protected Map<String, Object> _meta = new HashMap();
}
